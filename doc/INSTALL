...
4. PEAR Modules
   ...
   .. Important:: Make sure you are running a supported (i.e. new enough)
                  ...
                  "`Testing Horde Groupware`_".  Do **not** use the PEAR
                  version from ftp.horde.org.
...
2. Configuring Horde Groupware
   ...
   .. Warning:: All configuration files in Horde Groupware are PHP scripts that
   ...
   ``gollem/config/backends.d/10-groupware.php`` for details.
...
3. Setting up alarm emails
   ...
   If not installing Horde Groupware through PEAR or if PEAR's ``bin_dir``
...
4. Testing Horde Groupware
   ...
   Once you have configured your webserver, PHP, and Horde Groupware, bring up
...
5. Securing Horde Groupware
   ...
   a. Global Passwords
      ...
      Some of Horde Groupware's configuration files contain passwords which
      ...
      ``config/``) are not readable to system users.  There are ``.htaccess``
      ...
      ``.htaccess`` and is configured to use them, and that the files in those
      directories are in fact inaccessible via the browser.

      An additional approach is to make Horde Groupware's configuration files
      owned by the user ``root`` and by a group which only the webserver user
   ...
   b. User Passwords
      ...
      The second channel is between the Web server and their authentication
      ...
      Horde Groupware to connect to the backend on ``localhost`` instead of on
   ...
   see the `doc/SECURITY`_ file.


Temporary Files
===============

Various Horde Groupware applications will generate temporary files in PHP's
temporary directory (see the ``General`` tab in the Horde setup).  For various
...
Note for international users: Horde Groupware uses GNU gettext to provide local
...
translation, please see the `doc/TRANSLATIONS`_ file for instructions.
...
for general problems and questions.

Lastly, Horde developers, contributors and users may also be found on IRC,
on the channel #horde on the Freenode Network (irc.freenode.net).

Please keep in mind that Horde Groupware is free software written by
...
.. _doc/TRANSLATIONS: ../../horde/doc/TRANSLATIONS
